// French mock data for dashboards (company / talent / admin)

export type PipelineStage = "Nouveau" | "Présélection" | "Entretien" | "Finaliste" | "Recruté";
...
export type Candidate = {
  ...
  challenges: number;
  badges: string[];
  availability: "Immédiate" | "1 mois" | "3 mois";
};
...
export type CompanyPosition = {
  id: string;
  title: string;
  type: "Stage" | "Alternance" | "CDI" | "Freelance";
  city: string;
  remote: boolean;
  candidates: number;
  status: "Ouvert" | "En pause" | "Clôturé";
  openedAt: string;
};
...
export const COMPANY_CHALLENGES: CompanyChallenge[] = [
  {
    id: "cc-1",
...
export type Submission = {
  ...
  challenge: string;
  score: number;
  status: "Envoyée" | "En revue" | "Acceptée" | "Refusée";
  date: string;
};
...
export type Application = {
  ...
  company: string;
  emoji: string;
  status: "Envoyée" | "Vue" | "Entretien" | "Finaliste" | "Acceptée" | "Refusée";
  date: string;
};
...
export type ModerationItem = {
  id: string;
  type: "Signalement" | "Contenu suspect" | "Vérification";
  target: string;
  reason: string;
...
export type AdminCompany = {
  ...
  logo: string;
  sector: string;
  status: "Vérifiée" | "En attente" | "Suspendue" | "Refusée";
  challenges: number;
  hires: number;
  joined: string;
};
...
export const ADMIN_TALENTS: AdminTalent[] = [
  {
    id: "at-1",
    name: "Amélie Laurent",
    avatar: "👩🏻‍🎨",
