...
export const Route = createFileRoute("/company/messages")({ component: CompanyMessages });
...
function CompanyMessages() {
  ...
  return (
    <DashboardShell title="Messages" badge="Espace entreprise" items={COMPANY_NAV}>
      <div className="grid h-[calc(100vh-180px)] gap-4 md:grid-cols-[320px_1fr]">
        <div className="rounded-3xl border border-border bg-card p-3 flex flex-col">
          ...
          <ul className="mt-3 flex-1 overflow-y-auto space-y-1">
            {MESSAGES.map((m) => (
              <li key={m.id}>
                ...
                >
                  <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-accent text-xl">
                    {m.avatar}
                  </span>
                  ...
                  {m.unread && <span className="h-2 w-2 shrink-0 rounded-full bg-coral" />}
                </button>
              </li>
            ))}
          </ul>
